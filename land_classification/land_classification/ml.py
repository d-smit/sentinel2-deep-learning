...
def classify(df, pred_path='data/masked.tif', cv=True, name='mlp', algorithm=MLPClassifier()):
    ...
    write_raster("lc_10m_{}_proba.tif".format(name), proba, profile)
    
    if cv:
        cls_cv = cls.predict(X_test)
        score = cls.score(X_test, y_test)
        f, ax = pl.subplots(1, figsize = (20, 20))
        sns.heatmap(ax = ax, data=confusion_matrix(cls_cv, y_test), annot=True, fmt='g', linewidths=0.5, cbar=False)
        f.savefig('cv_{}.png'.format(name))